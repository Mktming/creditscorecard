#utf-8  Jiaming  2020-04-29 19:37:52
...
if __name__ == '__main__':
    ...
    data['NumberOfTime3059DaysPastDueNotWorse'] = Series(replace_woe(data['NumberOfTime3059DaysPastDueNotWorse'], cutx3, woex3))
    ...
    data['NumberOfTime6089DaysPastDueNotWorse'] = Series(replace_woe(data['NumberOfTime6089DaysPastDueNotWorse'], cutx9, woex9))
    ...
    #计算分数
    #coe为逻辑回归模型的系数
    coe=[7.885545,0.811083,0.494714,1.529214,2.286435,0.612646,1.524416]
    # 我们取600分为基础分值，PDO为20（每高20分好坏比翻一倍），好坏比取20。
    p = 20 / math.log(2)
    q = 300 - 20 * math.log(20) / math.log(2)
    ...
    test1['x3'] = Series(compute_score(test1['NumberOfTime3059DaysPastDueNotWorse'], cutx3, x3))
    test1['x7'] = Series(compute_score(test1['NumberOfTimes90DaysLate'], cutx7, x7))
    test1['x8'] = Series(compute_score(test1['NumberRealEstateLoansOrLines'], cutx8, x8))    
    test1['x9'] = Series(compute_score(test1['NumberOfTime6089DaysPastDueNotWorse'], cutx9, x9))
    test1['Score'] = test1['x1'] + test1['x2'] + test1['x3'] + test1['x7'] +test1['x8']+test1['x9']  + baseScore
    test1.to_csv('cs-tset-score.csv', index=False)
    plt.show()